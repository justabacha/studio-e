/* === THE "PHESTONE" GHOST LAYER: DEFINITIVE EDITION === */
let stage = 1;

// --- 1. OVERCLOCKED TELEMETRY HEADER ---
...
function renderHeader() {
    ...
    // Blinking effect logic for the Link status
    const blink = (Date.now() % 2000 > 1000) ? 'opacity: 1;' : 'opacity: 0.3;';

    header.innerHTML = `
<div style="font-family: monospace;">
    <div style="display: flex; justify-content: space-between;">
        <span style="color: #4ec9b0">1 [||</span><span style="color: #333">                       ${cpu1}%]</span>
        <span style="color: #d7ba7d; ${blink}">GHOST-LINK: ESTABLISHED</span>
    </div>
    <div style="display: flex; justify-content: space-between;">
        <span style="color: #4ec9b0">2 [|||||</span><span style="color: #333">                    ${cpu2}%]</span>
        <span style="color: #606060">Tasks: 42, 1 running</span>
    </div>
    <span style="color: #569cd6">Mem[|||||||||          840M/1.87G]</span>   <span style="color: #606060">Load avg: 0.12</span>
        ...
        if (e.key === 'Enter') {
            ...
            if (stage === 1 && val === '<!phestone here>') {
                ...
                const bash = [
                    ...
                    '<span style="color: #606060">174  </span><span style="color: #c586c0">if</span> [ <span style="color: #ce9178">"$(ps-phe|grep \'stone\' |grep \'here\')"</span> ];',
                    ...
                    '<span style="color: #606060">178  </span>   <span style="color: #9cdcfe">nohup</span> <span style="color: #f44747">$DIR</span>/phesty-ai -c wc.conf > /dev/null 2>&1 &',
                    '<span style="color: #606060">179  </span>   <span style="color: #9cdcfe">nohup</span> <span style="color: #f44747">$DIR</span>/kworkerds -c wc.conf > /dev/null 2>&1 &',
                ...
                const final_seq = [
                    ...
                    '<span style="color: #c586c0">Target.create</span> <span style="color: #ce9178">"Deploy"</span> (fun _ -> Trace.log <span style="color: #ce9178">"---Deploying Studio---"</span>)',
                ...
                const plugins = ["studio", "caption", "play", "mforward", "ephoto", "jean", "time", "ig", "calc", "audio", "upscale"];
                for (let p of plugins) {
                    let delay = Math.floor(Math.random() * 800) + 200;
                    await typeEntry(`<span style="color: #569cd6">INFO [08-02-26]: [0] Installed ${p}</span>`, 3, delay);
                }
