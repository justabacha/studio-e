...
function handleMagic() {  
    ...
    if (!val) return;  
    if (magicMode === 'AI') { callGroq(val); input.value = ""; }   
    else {  
        if (waStep === 1) {   
            sName = val;   
            document.getElementById('magic-title').innerText = `What's the word, ${sName}?`;   
            input.value = ""; input.placeholder = "Talk to me...";   
            ...
            document.getElementById('magic-title').innerText = "Vibe Sent! 🚀"; input.value = "";   
        }  
    }  
}  
